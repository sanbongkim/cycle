...
//  testIOS
//
//  Created by LIM JAEHYO on 2020/08/05.
//  Copyright © 2020 LIM JAEHYO. All rights reserved.
//
...
class UnityEmbeddedSwift: UIResponder, UIApplicationDelegate, UnityFrameworkListener  {
    ...
    static func showUnity( _ callBack : UnityInit , _ eventCallBack : NativeCallsProtocol) {
        if(UnityEmbeddedSwift.instance == nil || UnityEmbeddedSwift.instance.unityIsInitialized() == false) {
            UnityEmbeddedSwift().initUnityWindow(callBack,eventCallBack)
        }
        else {
            UnityEmbeddedSwift.instance.showUnityWindow(callBack)
        }
    }
    ...
    private func initUnityWindow(_ callBack : UnityInit, _ eventCallBack : NativeCallsProtocol) {
        if unityIsInitialized() {
            showUnityWindow(callBack)
            return
        }
        ...
        NSClassFromString("FrameworkLibAPI")?.registerAPIforNativeCalls(eventCallBack)
        ...
        initUnityWindow(callBack,eventCallBack)
    }
    
    private func showUnityWindow(_ callBack : UnityInit) {
        if unityIsInitialized() {
            ufw.showUnityWindow()
            sendUnityMessageToGameObject()
            callBack.unityLodingCall()
        }
    }
